...
- (void)cleanup {
 ...
 /// root_window_->OnNativeWindowClosed(); -     /// CHROMELYPARAM- function pointer?
  ...
  //think i may need ot add these here to cause the shutdown to kickin? atm its hanging
  //chromelyParam_.cefShutdownCallback();
...
- (void)createApplication:(id)object {
  ...
  window_ = [[window_class alloc]
    ...
    chromelyParam_.createCallback((__bridge void *)window_, (__bridge void *)cefParentView_);
...
- (void)tryToTerminateApplication:(NSApplication*)app {
    NSLog(@"[libchromely] tryToTerminateApplication");
...
//        NSLog(@"[libchromely] retaining CefBrowserHostView");
//
//        // retraining to try and fix intermittent crash - possible memory leak?
//        //         0   libcef                        	0x000000011b03abb7 CefBrowserHostImpl::WindowDestroyed() + 183 (browser_host_impl.cc:1560)
//        //         1   libcef                        	0x000000011b19a001 -[CefBrowserHostView dealloc] + 33 (browser_platform_delegate_native_mac.mm:55)
//        //         2   libobjc.A.dylib               	0x00007fff6c50b054 AutoreleasePoolPage::releaseUntil(objc_object**) + 134
//        //         3   libobjc.A.dylib               	0x00007fff6c4efdba objc_autoreleasePoolPop + 175
//        [view retain];
//        NSLog(@"[libchromely] removing from subview");
//
...
//    // Delete the window.
//    #if !__has_feature(objc_arc)
//      NSLog(@"[libchromely] no ARC - window autoreleas");
//      [window_ autorelease];
//    #endif  // !__has_feature(objc_arc)
//      window_ = nil;
//
...
//    NSLog(@"[libchromely] dealloced & exited");
//}
...
void createwindow(CHROMELYPARAM* pParam) {
      ...
      [appDelegate performSelectorOnMainThread:@selector(createApplication:)  
        ...
        NSLog(@"[libchromely] runloop ended");
...
//        [NSAutoreleasePool showPools];
        ...
        #if !__has_feature(objc_arc)
            NSLog(@"[libchromely] appDelegate release");
            [appDelegate release];
        #endif  // !__has_feature(objc_arc)
        appDelegate = nil;
    }  // @autoreleasepool
...
APPDATA createwindowdata(CHROMELYPARAM* pParam) {
    ...
    APPDATA appData;
    appData.app = (__bridge void*)NSApp;
   // appData.pool = pool;

    return appData;
}

void run(NSApplication* application) {
    [application run];
}

void quit(NSApplication* application, void* pool) {
   // [pool release];
